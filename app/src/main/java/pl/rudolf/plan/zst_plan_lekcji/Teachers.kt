...
import kotlinx.android.synthetic.main.activity_teachers.progressTeachers
...
class Teachers : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.title = getString(R.string.action_teachers)
        downloadClasses()
        longToast("Ladowanie...")
    }
    ...
    private fun downloadClasses(){
        doAsync {
            try {
                ...
                val body =doc.getElementById("nauczyciele").select("a")
                body.forEach{item->
    ...
    private fun loadOffline():MutableList<String>{
        try {
            offlineTeachers.addAll(getList("teachers")
                    .replace("[","")
                    .replace("[","")
                    .split(","))
    ...
    private fun render(){
        val lv = listView {  }
        val arrayClasses = java.util.ArrayList<String>()
        arrayClasses.addAll(listOfLinks)
        customAdapter = ArrayAdapter(this@Teachers,
        ...
        lv.setOnItemClickListener{parent,_,position,_->
            ...
            val teacherName = tmp.substring(tmp.indexOf(" "),tmp.length)
            startActivity(Intent(this, ShowPlan::class.java)
                    .putExtra("address", "n"+tmp.split(" ")[0]+".html")
                    .putExtra("name", teacherName)
                    .putExtra("date",date))
        }
    }
