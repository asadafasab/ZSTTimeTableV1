...
class Classrooms : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.title = getString(R.string.action_classrooms)
        downloadClasses()
        longToast("Ladowanie...")
    }
    ...
    private fun downloadClasses(){
        doAsync {
            try {
                ...
                val body =doc.getElementById("sale").select("a")
                body.forEach{item->
    ...
    private fun loadOffline():MutableList<String>{
        try {
            offlineTeachers.addAll(getList("classrooms")
                    .replace("[","")
                    .replace("[","")
                    .split(","))
    ...
    private fun render(){
        ...
        customAdapter = ArrayAdapter(this@Classrooms,
        ...
        lv.setOnItemClickListener{parent,_,position,_->
            ...
            startActivity(Intent(this, ShowPlan::class.java)
                    .putExtra("address", "s"+tmp.split(" ")[0]+".html")
